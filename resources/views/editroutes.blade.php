        ...
        <div class="content__container">
            <div class="edit-point__forms">
                <form method="post" action ="{{route('UpdateRoute',$_SESSION['CurrentEditRoute']->id)}}" id="edit-form" enctype="multipart/form-data">
                    ...
                    <input  {{$fieldAccess->distance}} type="number"  step = "any" min="0"  placeholder="Введите протяженность" name="distance" value ="{{$_SESSION['CurrentEditRoute']->distance}}" >

                    <h4 class="sub-title">Примерное время, ч</h4>
                    <input  {{$fieldAccess->time}}  type="number"  step = "any"min="0"  placeholder="Введите время" name="time" value = "{{$_SESSION['CurrentEditRoute']->time}}">
